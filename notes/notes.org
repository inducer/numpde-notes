...
#+BEAMER_HEADER: \newcommand{\demolink}[2]{\plainhref{https://mybinder.org/v2/gh/illinois-scicomp/cs555-s20-binder/master?filepath=#1/#2.ipynb}{\color{purple}\uline{\textbf{Demo: }#2}}}
...
\begin{eqnarray*}
  0 & = & \frac{u_{j, \ell + 1} - u_{j, \ell}}{h_t} + a \frac{u_{j, \ell} -
  u_{j, \ell - 1}}{h_x}\\
  & = & \frac{u_{j, \ell + 1} - u_{j, \ell}}{h_t} + \frac{f (u_{j, \ell}) - f
  (u_{j, \ell - 1})}{h_x}\\
  & = & \frac{u_{j, \ell + 1} - u_{j, \ell}}{h_t} + \frac{f^{\ast} (u_{j,
  \ell}, u_{j + 1, \ell}) - f^{\ast} (u_{j, \ell - 1,} u_{j, \ell})}{h_x} .
\end{eqnarray*}
...
\[ \frac{u_{j, \ell + 1} - u_{j, \ell}}{h_t} + \frac{f^{\ast} (u_{j, \ell},
   u_{j + 1, \ell}) - f^{\ast} (u_{j, \ell - 1,} u_{j, \ell})}{h_x} \]
with
